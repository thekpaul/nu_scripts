...
If deemed mature enough, these may even be formatted into [Pull Requests][up_PR]
to be submitted upstream.
"Feature" branches will only be removed if successfully merged to upstream as
Pull Requests, and the replacing commits upstream will be merged into this fork.

Commits from the upstream repository [`nushell/nu_scripts`][upstream] will also
be merged into the [`main`][fork] branch of this fork spontaneously.
